...
const SoroPass: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (localStorage.hasOwnProperty("sp:bundler")) {
      ...
      const horizon = new Horizon.Server(
        import.meta.env.VITE_PUBLIC_horizonUrl!
      );
  ...
  const onSign = useCallback(async () => {
    try {
      ...
      const respone = await handleVoteSend(
        bundlerKey!,
        authTxn,
        lastLedger,
        signRes
  ...
  const tapHandler = (event: any) => {
    if (!["div", "h1", "p"].includes(event.detail.target.tagName.toLowerCase()))
      return;
    else if (
  ...
  //       dialogTitle: `${
  //         choice === "chicken" ? "Chicken 🐔" : "Egg 🥚"
  //       } people unite!`,
  //     });
  //   } else {
  //     window.open(
  //       `https://twitter.com/intent/tweet?text=${encodeURIComponent(
  //         "Check out this blockchain experience powered by your face or fingers!"
  //       )}&url=${encodeURIComponent("https://passkey.sorobanbyexample.org/")}`
  //     );
  //   }
  // };
  ...
  return (
    <div
      ...
      className={`relative w-full flex flex-col items-center justify-center h-dvh px-2 select-none overflow-hidden ${
        !Capacitor.isNativePlatform() ? "max-h-[800px] max-w-[500px] py-2" : ""
      ...
      onClick={(e) =>
        tapHandler({ detail: { target: e.target, x: e.clientX } })
      }
    >
